...
- real-eval、review、promotion 是 harness 的反馈闭环，而不是外围测试装饰。
...
5. harness core / protocol / app server
...
3. `src/harness/server/harness-session-registry.ts`
   为不同 scope 组装并缓存 runtime session
4. `src/harness/core/session/harness-session.ts`
...
5. `src/harness/core/session/session-kernel.ts`
...
- 产品主路径已经稳定为：
  `main.ts -> runtime-daemon -> harness-session-registry -> harness protocol/app server -> default TUI surface`
