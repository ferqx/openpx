...
/** 持久化层装配结果：sqlite + stores + runStateStore */
type PersistenceLayer<TSqlite, TStores> = {
  sqlite: TSqlite;
  stores: TStores;
};
...
export async function createAppPersistenceLayer<TSqlite, TStores>(input: {
  config: ConfigLike;
...
export async function closeAppContextResources(input: {
  ...
  // 关闭顺序上优先释放 stores，再关闭底层 sqlite 句柄，
