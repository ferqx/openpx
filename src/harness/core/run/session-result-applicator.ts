/**
 * @module harness/core/run/session-result-applicator
 * harness 会话结果应用器（session result applicator）。
 *
 * 它负责把 control plane 返回的结果落实到 durable thread 真相，
 * 并生成 narrative、answer、tool 状态等恢复所需信息。
 */
...
import type { SessionControlPlaneResult } from "../session/session-kernel";
...
/** 把 control plane 结果应用到 thread 真相。 */
