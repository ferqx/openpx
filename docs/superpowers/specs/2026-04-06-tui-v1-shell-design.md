...
1. 主流中的近期对话与流式输出
2. 当主流为空时，由 durable answer 或 narrative summary 提供回填摘要
...
- 如果当前空闲，`Esc` 直接退出
- 如果当前仍在执行，TUI 需要先向 runtime 发出 interrupt / stop 语义，再退出
- 如果中断失败，退出前至少需要给用户一条清晰反馈

关键目标是避免“用户以为退出即停止，实际后台仍继续运行”的语义错位。
...
2. 再完善 Composer 与 `/` 命令解析
3. 再增强 Main Stream 与当前线程历史能力
4. 最后补齐简洁的 Context Drawer 和 settings/help 视图
