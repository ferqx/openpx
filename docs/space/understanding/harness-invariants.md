...
- executor 的项目外读、写和命令执行必须先形成 approval，且摘要必须暴露“项目目录之外”的风险与目标绝对路径
...
- approval 后恢复与 continuation 恢复必须遵守幂等或可判定语义
- `executeApproved()` 只能执行已审批的同一个真实路径目标，符号链接、`../`、同前缀路径或外部 `cwd` 解析结果变化都必须阻止执行
- harness 必须能解释“哪些动作已发生，哪些动作只是待恢复”
- execution ledger 必须优先参与“不确定副作用”判定；无法确认时必须转 `human_recovery`
...
- 没有真实 `tool.executed` 和 artifact 时，不得生成或声称 `patch:*` 产物
